...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def validate_email_phone_number(self, value):
        ...
        if value and User.objects.filter(email=value).exists():
            ...
            data = {
                "success": False,
                "message": "Bu telefon raqam allaqachon malumotlar bazasida bor"
            }
            raise ValidationError(data)
        return value
...
class ChangeUserInformation(serializers.Serializer):
    ...
    def validate_username(self, username):
        if len(username)<5 or len(username)>30:
            raise ValidationError(
                {
                    "message": "Username 5-30 ta belgidan tashqil topishi kerak"
                }
            )
        if username.isdigit():
            raise ValidationError(
                {
                    "message":"Username faqat raqamlardan tashqil topishi mumkun emas"
                }
            )
        return username
    ...
    def validate_last_name(self, last_name):
        if len(last_name)<5 or len(last_name)>30:
            raise ValidationError(
                {
                    "message":"last_name 5 ta simvoldan kop va 30 ta simvoldan kam bolishi kerak"
                }
            )
        if last_name.isdigit():
            raise ValidationError(
                {
                    "message":"last_name faqat raqamlardan iborat bolishi mumkun emas"
                }
            )
        return last_name
    ...
    def validate_first_name(self, first_name):
        if len(first_name)<5 or len(first_name)>30:
            raise ValidationError(
                {
                    "message":"first_name 5 ta simvoldan kop va 30 ta simvoldan kam bolishi kerak"
                }
            )
        if first_name.isdigit():
            raise ValidationError(
                {
                    "message":"first_name faqat raqamlardan iborat bolishi mumkun emas"
                }
            )
        return first_name
...
class LoginSerializer(TokenObtainPairSerializer):
    ...
    def auth_validate(self, data):
        ...
        if check_user_type(user_input) == 'username':
            ...
            user = self.get_user(email__iexact=user_input)   #User.objects.get(email__iexact=user_input) shunaqa korinishdaxam yozsak boladi
            username = user.username
        elif check_user_type(user_input) == 'phone':
            user = self.get_user(phone_number=user_input)     #User.objects.get(phone_number=user_input) shunaqa korinishdaxam yozsak boladi
            username = user.username
        else:
            data = {
                'success': True,
        ...
        print(current_user)
        if current_user != None and current_user.auth_status in [NEW, CODE_VERIFIED]:
            raise ValidationError(
                {
                    "success":False,
